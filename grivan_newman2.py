...
def getModularity(G, layer, node_l, node_c, top, bot, couple, edge_l, edge_c, mu, communities):
    status = Status()
    status.layer=layer
    status.node_l=node_l
    status.node_c=node_c
...
def newmann_girvain(G, layer, node_l, node_c, top, bot, couple, edge_l, edge_c, mu, commu):
    ...
    while True:
        ...
        Q = getModularity(originalG, layer, originalnode_l, originalnode_c, top, bot, couple, edge_l, edge_c, mu, communities);
        
        #Q = getModularity(G, layer, node_l, node_c, top, bot, couple, edge_l, edge_c, mu);
        #print "Modularity of decomposed G: %f" % Q
        if Q > maxQ:
            maxQ = Q
            best_communities = list(nx.connected_components(G))
    ...
    return commu_list, modu_list, edges_p_list, best_communities, maxQ
...
def main():
    ...
    modfilename = pathtosave+"grivanNewman_Qm_2"
