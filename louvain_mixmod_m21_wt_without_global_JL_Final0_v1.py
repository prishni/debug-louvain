...
def __one_level(graph, status, status_list, level_count, verbose=0) :
	...
	while modif and nb_pass_done != __PASS_MAX :
		...
		for node in v:
			...
			base_mod = __modularity(_get_com_wise_nodes(status.node2com), status, graph)
			if ffff==0:
			...
			if node in group:
				for node1 in group[node]:
					neigh_communities.extend(__neighcom(node1, graph, status))
			'''		
			...
			for com in neigh_communities:
				...
				incr = __modularity(_get_com_wise_nodes(status.node2com), status, graph) - base_mod
...
def louvain(graph, layer, node_l, node_c, top, bot, couple, edge_l, edge_c, mu) :
	...
	new_mod = __modularity(_get_com_wise_nodes(part), status, current_graph)
...
#		A = nx.adjacency_matrix(current_graph)
	...
	print _get_commu_dict(partition_at_level(status_list, level_count))
...
for networktype in networktypes:
	for algoname in algonames:
		...
		modfilename="./results/"+algoname+"/final0_v1/"+str(networktype)+"/Mixmod_global_modu_not_global_low_couple_Final0_v1.txt"
