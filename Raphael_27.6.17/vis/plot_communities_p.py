...
olddir = '../synthetics/'
oldfileslist = os.listdir(olddir)
'''
for filename in oldfileslist:
    oldnetwork,communities = readtextnetwork(olddir+filename)
    communities, oldnetwork, translation = align_communities(communities, oldnetwork)
    draw_graph_from_nx(oldnetwork, communities, 'img/'+filename+'.png', filename)

'''
filename = 'network_0.9_1.0_0.05_1.0_0.0'
oldnetwork, communities = readtextnetwork(olddir+filename)
communities, oldnetwork, translation = align_communities(communities, oldnetwork)
draw_graph_from_nx(oldnetwork, communities, filename+'.png', filename)

#Draw detected communities
f = open('../../_commu_benching_all_march21_louvain_mixmod.pickle', 'rb')
p = pickle.load(f)
m = max(p.values())
louvain_p = defaultdict(list)
for l in p.keys():
    louvain_p[p[l]].append(translation[l])
draw_graph_from_nx(oldnetwork, louvain_p, "detected_mixmod.png", "detected_mixmod")


# In[2]:
#Draw graph for new networks (files taken from pickle files)
sys.exit(0)
