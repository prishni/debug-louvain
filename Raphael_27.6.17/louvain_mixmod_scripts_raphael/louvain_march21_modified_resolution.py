        ...
        if len(intra_inter[c])>1:
            for l in layer:
                ...
                if I_layer>-1 and m_layer>0:
                    mod=((I_layer/m_layer)-gamma*((float(d_layer)/(2*m_layer))*(float(d_layer)/(2*m_layer))))
                else:
                    mod=0   
                #if n_layer > 0:
            ...
            for co in couple:
                ...
                if I_couple>-1 and (m_couple+m_layer_top+m_layer_bot)>0:
                    ...
                    mod=((I_couple/(m_couple+m_layer_top+m_layer_bot))-gamma*((d_couple_top*d_couple_bot)/((m_couple+2.0*m_layer_top+2.0*m_layer_bot)*(m_couple+2.0*m_layer_top+2.0*m_layer_bot))))
                else:
                    mod=0   
            ...
            for l in layer:
                ...
                if I_layer>-1 and (1.0*m_layer+(c_layer/2.0))>0:# and m_layer1>0:
                    ...
                    mod=((I_layer/(1.0*m_layer+(c_layer/2.0)))-gamma*((float(d_layer)/(2.0*m_layer+c_layer))*(float(d_layer)/(2.0*m_layer+c_layer))))
                    #else:
...
def louvain(graph, layer, node_l, node_c, top, bot, couple, edge_l, edge_c, mu) :
    ...
    mod = new_mod
    with open(str2+'_commu_benching_all_march21_louvain_resolution_step1.pickle', 'wb') as handle:
...
with open(str2+'_commu_benching_all_march21_louvain_resolution_6.0.pickle', 'wb') as handle:
