...
def __one_level(graph, status, status_list, level_count, verbose = 0) :
    ...
    while modif  and nb_pass_done != __PASS_MAX :
        ...
        for node in graph.nodes():
            ...
            if(verbose):
                print("Node ", node, " moved from ", com_node, " to ", best_com, " increase: ", best_increase)
...
def __git_one_level(graph, status, weight_key, resolution, randomize):
...
def getSeries(filename):
    ...
    louvain(ml_network, layer, node_l, node_c, top, bot, couple, edge_l, edge_c, mu)
