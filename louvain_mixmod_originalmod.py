...
import pickle
import math
from mixmod_originalmod.Status import Status
from mixmod_originalmod.modularity import __modularity
from collections import defaultdict

#import matplotlib.pyplot as plt
from mixmod_originalmod.aux import _get_com_wise_nodes, printsomeinformation
...
def induced_graph_multilayer(partition, graph, status):
    print("partition:",_get_com_wise_nodes(partition))
...
def computegtmod(filename):
    ...
    layer,node_c = makesinglelayer(layer,node_l, node_c, top, bot)
...
def makesinglelayer(layer,node_l, node_c, top, bot):

    #merge node_l and node_c-----------------------------------
    for n in node_c:
        if n not in node_l:
            node_l[n] = node_c[n]
        else:
            #print(type(node_l[n]), type(node_c[n]))
            for node in node_c[n]:
                node_l[n].add(node)
    ...
    key=1
    for l in layer:
        if key not in layersingle:
            layersingle[key] = layer[l]
        else:
            for node in layer[l]:
                layersingle[key].add(node)

    layer = layersingle
    node_c = {}
    return layersingle,node_c
...
pathtosave = './mixmod_originalmod/results_mixmod_originalmod/'
modfile = open(pathtosave+"modComparisionMixModLouvain_wt_originalmodularity",'w')
...
for network in networklist:
    ...
    modfile = open(pathtosave+"modComparisionMixModLouvain_wt_originalmodularity",'a')
