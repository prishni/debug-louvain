...
def __one_level(original_graph, original_status, graph, status, status_list, level_count, verbose=0) :
	...
	for node1 in status.node2com:
		...
		#node2group[node1] =com1
		if com1 not in group:
			group[com1] = set()
		group[com1].add(node1)
		for node2 in status.node2com:
			com2=status.node2com[node2]
			if node1!=node2 and com1==com2:
				group[com1].add(node2)
				#if node2 not in node2group:
				#	node2group[node2]= com1
	...
	while modif  and nb_pass_done != __PASS_MAX :
		...
		for node in v:
			...
			prev_com = status.node2com[node]
			#print("for {0}, prev_com: {1}, group: {2}".format(node,prev_com,group[prev_com]))
			...
			if node in status.node2com:
				for node1 in group[prev_com]:
			...
			if (move_in_group == 1):
				...
				group[best_com] = group[best_com].union(group[prev_com])
				if node in status.node2com:
					for node1 in group[prev_com]:
						status.node2com[node1] = best_com
						#node2group[node1] = best_com
				group.pop(prev_com,None)
			else:
				#print(prev_com)
				#pprint(group[prev_com])
				#print(node)
				group[prev_com].remove(node)
				group[best_com].add(node)
				status.node2com[node] = best_com
			#node2group[node] = best_com
...
def runformanynetworks(args):
	...
	for network in networklist:
		...
		comfile = open(p+"detectedCommuMixmodUnion",'a')
		comfile.write(str2+":\n============================================================\n")
		for d in dtcom:
			towrite = ' '.join([str(node) for node in d])
...
def parallelimplementation(networklist,weighted,modfilename,networkpath):
	...
	comfile = open("detectedCommuMixmodUnion",'w')
...
def main():
	...
	modfilename = pathtosave+"mixmod_union"
